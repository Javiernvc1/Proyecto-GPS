...
async function forgotPassword(req, res) {
    try {
        ...
        const [user, errorToken] = await AuthService.forgotPassword(email);
        if (errorToken) {
...
async function resetPassword(req, res) {
    try {
        ...
        if (newPassword !== confirmPassword) {
            return respondError(req, res, 400, "La nueva contraseña y la confirmación de contraseña no coinciden");
        }
