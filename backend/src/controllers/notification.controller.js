...
/**
 * Crea una nueva notificación utilizando el servicio 'NotificationService.createNotification()' con el parámetro de body
...
async function createNotification(req, res) {
    try {
        ...
        const [newNotification, error] = await NotificationService.createNotification(bodyNotification);
        if (error) return respondError(req, res, 400, error);
        respondSuccess(req, res, 201, newNotification);
    } catch (error) {
        handleError(error, 'notification.controller -> createNotification');
        respondError(req, res, 500, 'Error al crear la notificación');
    }
}
